...
export default function RegisterPage() {
  ...
  const handleRegisterWithEmail = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const profile = await initializeUser(user, name);

      if (profile.role === 'admin') {
         toast({ title: 'Account Created', description: 'You have been successfully registered.' });
      } else {
         toast({ title: 'Welcome Aboard!', description: `Your account is now active with the ${profile.role} role.` });
      }
  ...
  return (
    <Card className="w-full max-w-sm shadow-2xl">
      <CardHeader className="space-y-1 text-center">
        ...
        <CardDescription>
          Get started with PlotPilot today.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <form onSubmit={handleRegisterWithEmail} className="space-y-4 pt-2">
            <div className="space-y-2"><Label htmlFor="name">Full Name</Label><Input id="name" type="text" placeholder="John Doe" required disabled={isLoading || isGoogleLoading} value={name} onChange={(e) => setName(e.target.value)} /></div>
            <div className="space-y-2"><Label htmlFor="email">Email</Label><Input id="email" type="email" placeholder="name@example.com" required disabled={isLoading || isGoogleLoading} value={email} onChange={(e) => setEmail(e.target.value)} /></div>
            <div className="space-y-2"><Label htmlFor="password">Password</Label><Input id="password" type="password" required disabled={isLoading || isGoogleLoading} value={password} onChange={(e) => setPassword(e.target.value)} /></div>
